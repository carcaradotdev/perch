...
Then declare routes as classes annotated `@DeepLink`, under a route type of your own:
...
class PaymentLink(val id: String) : Route
```

`Route` is yours. Perch has no supertype for routes to implement, and nothing in the build scripts
names it: the processor reads the type your routes share and generates
`DeepLinkParser<Route>.registerDeepLinks()` against it. Nor is `@Serializable` needed — `@DeepLink`
is `@MetaSerializable`, so the kotlinx.serialization compiler plugin generates the serialiser from
it alone.
...
When the routes share several supertypes, the most specific one wins. When they share none, or none
of the shared ones is the most specific, the build fails and names the candidates rather than
guessing; settle it with `perch { targetBaseClass.set("com.example.routes.Route") }`.
 ...
 */
public fun DeepLinkParser<com.example.routes.Route>.registerAllDeepLinks() {
  register<com.example.routes.HomeLink>()
  register<com.example.routes.PaymentLink>()
}
```

The route type comes from the producers' manifests, so the aggregating module names it only when
the producers it reaches disagree — then
`perchAggregation { targetBaseClass.set("com.example.routes.Route") }` picks the type they share.
...
fun appParser(): DeepLinkParser<Route> =
    DeepLinkParser<Route>(schemes = setOf("myapp", "https"), hosts = setOf("myapp.example"))
...
`parse` returns your `Route`, so a sealed one makes that `when` exhaustive. That is the whole
surface: a typed target, or `null`. Deciding when to act on it, how to navigate,
